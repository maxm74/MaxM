...
uses
  {$ifdef RTDebug}
    RTDebug,
  {$endif}
  SysUtils, Classes, Graphics, Messages, LMessages, Controls,
...
type
  ...
  protected
    ...
    rBlinkStopAfter,
    BlinkCounter :DWord;
    MouseWindow  :TMouseWinControl;

    procedure DoParentMouseDown(Button: TMouseButton; Shift: TShiftState;  X, Y: Integer);
    procedure DoParentMouseMove(Shift: TShiftState; X, Y: Integer);
    procedure DoParentMouseUp(Button: TMouseButton; Shift: TShiftState;  X, Y: Integer);

    //l function GetPalette: HPALETTE; override;
    procedure SetEnabled(Value : Boolean); override;
    procedure DrawButtonText(ACanvas: TCanvas; const ACaption: String; TextBounds: TRect; State: TButtonState);
    procedure MouseDown(Button: TMouseButton; Shift: TShiftState;  X, Y: Integer); override;
    procedure MouseMove(Shift: TShiftState; X, Y: Integer); override;
    procedure MouseUp(Button: TMouseButton; Shift: TShiftState;  X, Y: Integer); override;
...
//l    OldBkColor := SetBkColor(ColorBmp.Canvas.Handle, ColorToRGB(TransparentColor));
...
//l    SetBkColor(ColorBmp.Canvas.Handle, OldBkColor);
    MakeMask := TmpBitmap;
  except
    TmpBitmap.Free;
  end;
...
procedure Register;
begin
     RegisterComponents('MaxM_UI', [TJetButton]);
end;
...
(* //l Windows Version
  if not (ssLeft in Shift) then
    if MouseOver
      then begin
              SetCapture(MouseWindow.Handle);
              {$ifdef RTDebug}
                RTAssert(0, true, 'MOVE SET ' + Self.Name, '', 0);
              {$endif}
           end
      else begin
              ReleaseCapture();
              {$ifdef RTDebug}
                RTAssert(0, true, 'MOVE REL ' + Self.Name, '', 0);
              {$endif}
           end;
*)
  ...
  if MouseOver
    ...
    else DoParentMouseMove(Shift, X, Y);
end;

procedure TJetButton.MouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  MouseOver :=PtInMask(X, Y);

  MouseDownInsideMask := MouseOver;
...
(* //l Windows Version
            if (GetCapture <> MouseWindow.Handle) then
            begin
                 SetCapture(MouseWindow.Handle);
                 {$ifdef RTDebug}
...
*)
  ...
  else DoParentMouseDown(Button, Shift, X, Y);
end;

procedure TJetButton.MouseUp(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  MouseOver :=PtInMask(X, Y);
  ...
  if MouseOver
    then begin
            ...
            if MouseDownInsideMask
              ...
              else DoParentMouseUp(Button, Shift, X, Y);
         end
    else DoParentMouseUp(Button, Shift, X, Y);
end;

procedure TJetButton.DoParentMouseDown(Button: TMouseButton; Shift: TShiftState;  X, Y: Integer);
Var
   Pt :TPoint;
...
procedure TJetButton.DoParentMouseMove(Shift: TShiftState; X, Y: Integer);
Var
   Pt    :TPoint;
...
procedure TJetButton.DoParentMouseUp(Button: TMouseButton; Shift: TShiftState;  X, Y: Integer);
Var
   Pt :TPoint;
...
begin
  ...
  NeedRepaint := not MouseOver and Enabled and (DragMode <> dmAutomatic); //l and (GetCapture = 0);
...
//l  if MouseOver
//l  then SetCapture(MouseWindow.Handle);
//  else ReleaseCapture;
...
const
  txtStyle :TTextStyle =(Alignment:taCenter; Layout:tlCenter; SingleLine:true);

procedure TJetButton.DrawButtonText(ACanvas: TCanvas; const ACaption: String;
...
begin
  ...
  begin
       ...
       inc(TextBounds.Left, 1);
       inc(TextBounds.Top, 1);
       inc(TextBounds.Right, 1);
       inc(TextBounds.Bottom, 1);
  end;

  ACanvas.TextRect(TextBounds, 1,1, CString, txtStyle); //DrawText(ACanvas.Handle, CString, -1, TextBounds, DT_CENTER or DT_VCENTER or DT_SINGLELINE);
end;
...
begin
  ...
  begin
    R := BoundsRect;
    Invalidate; // InvalidateRect(Parent.Handle, @R, True);
  end;
end;
...
constructor TGroupsListCollectionItem.Create(ACollection: TCollection);
begin
